...
호요랩(또는 게임 내) "캐릭터 정보" 화면에서 성유물 5부위가 보이는 부분을 그대로 드래그해서 복사한 뒤, "호요랩 붙여넣기로 등록" 칸에 붙여넣으면 자동으로 파싱해서 등록됩니다. 부위는 순서가 아니라 **성유물 개별 이름**(예: "진실 갈망의 꽃")으로 인식하기 때문에, `data.js`의 `PIECE_NAME_REGISTRY`에 등록된 이름만 자동 인식돼요.

```js
const PIECE_NAME_REGISTRY = {
  "진실 갈망의 꽃": { slotKey: "flower", setKey: "하늘 경계가 드러난 밤" },
  // 새 세트/이름을 알게 되면 이런 식으로 한 줄 추가
};
```

등록되지 않은 이름이 나오면 그 항목만 건너뛰고 나머지는 정상 등록되며, 결과 메세지에 어떤 이름을 못 찾았는지 알려줍니다. 장착 캐릭터는 자동으로 "캐릭터 스펙" 탭에서 현재 골라둔 캐릭터로 채워지고, 부옵션 4개 미만으로 인식되면 그 항목도 등록되지 않고 이유가 표시됩니다.
